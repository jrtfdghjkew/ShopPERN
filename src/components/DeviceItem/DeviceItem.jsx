...
const DeviceItem = ({gadget}) => {
    const navigate = useNavigate()
    const {device} = useContext(Context)
    const brand = device.brands.filter(i=>i.id === gadget.brandId)[0].name
    const type = device.types.filter(i => i.id === gadget.typeId)[0].name.slice(0, -1)
    return (
            ...
            <Card style={{width: '150px', cursor: 'pointer'}} border={"light"}>
                ...
                <div className="d-flex justify-content-between">
                    <div style={{overflowX: "hidden"}} className="text-black-50 d-flex flex-nowrap justify-content-sm-between">
                        <div style={{marginRight: '2px'}}>{type}</div>
                        <div>{brand}</div>
                    </div>
